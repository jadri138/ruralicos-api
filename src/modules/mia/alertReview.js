...
function construirItemRevisionMIA({
  ...
  return {
    ...
    digest_item: {
      ...
      score: digestItem?.selection_score ?? digestItem?.score ?? null,
      similarity_score: digestItem?.similarity_score ?? tags.similitud ?? null,
      selection_action: digestItem?.selection_action || tags.selection?.action || null,
      selection_reason: digestItem?.selection_reason || tags.selection?.motivo || null,
      selection_risk: digestItem?.selection_risk || tags.selection?.riesgo || null,
      motivo_seleccion: digestItem?.motivo_seleccion || null,
      resumen_usado: digestItem?.resumen_usado || null,
      decision_digest: digestItem?.selection_decision || tags.decision_digest || null,
    },
    user: user ? {
      id: user.id,
